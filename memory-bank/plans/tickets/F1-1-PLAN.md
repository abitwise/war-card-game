...
## Summary
Deliver an Ink-based TUI for `war play` (now the default UI) that renders the event stream live without prompt-based blocking. Keep the engine untouched while offering a richer view and ensuring autoplay compatibility, while still providing the prompt UI as an option.

## Requirements Alignment
- Based on ticket F1-1 in `memory-bank/plans/TICKETS.md` and architecture/product briefs.  
- Add a `--ui ink` option to `war play` while keeping the prompt UX available as a secondary option.  
...
## Plan / Tasks
1. Add CLI flag/validation for selecting UI mode (`ink` default, `prompt` optional) and pass autoplay/seed into the chosen runner.  
...
## Completion Notes
- Added an Ink-based play experience behind `--ui ink` (now the default), including autoplay handling, stats/help panels, and controls parity with the prompt UI.  
